...
@section('content')
  <h1>
    Edit Payment for 
    {{ optional($repayment->expectedSchedule->application->client)->name ?? 'N/A' }}
    ({{ \Carbon\Carbon::parse($repayment->expectedSchedule->due_date)->format('M-Y') }})
  </h1>

  <form action="{{ route('bookkeeper.repayments.update', $repayment) }}" method="POST">
    @csrf
    @method('PUT')
    <input type="hidden" name="expected_schedule_id" value="{{ $repayment->expected_schedule_id }}">
    ...
    <div class="d-flex gap-2">
      ...
      <a href="{{ route('bookkeeper.ledger.view', $repayment->expectedSchedule->loan_application_id) }}"
